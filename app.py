...
def kabinet_ai_generate():
    ...
    try:
        ...
        if gen_type == "title":
            prompt = f"""BUYMAの出品タイトルを半角56文字以内で作成。必ず全て日本語のみで作成。韓国語（ハングル文字）は絶対に使用禁止。ブランド名も韓国語ではなく英語またはカタカナで表記。タイトルのみ出力（説明不要）。
...
- カラー: {colors or '不明'}
- サイズ: {sizes or '不明'}
{f'- ユーザーからの追加指示: {keywords}（このキーワードを必ずタイトルに含めること）' if keywords else ''}

[タイトル作成ルール]
- タイトルにブランド名「{brand}」は入れない（BUYMAでは別途ブランド欄があるため、タイトルの文字数を他のキーワードに使う）
{f'- ユーザーからの追加指示「{keywords}」に必ず従うこと' if keywords else ''}- 商品の魅力が伝わるキーワードを最大限に入れる
- 以下のような注目キーワードを活用:
  【SALE】セール価格の場合
  【即発】手元在庫で即日発送可能な場合
  【size:S】などサイズ情報
...
[絶対禁止事項]
- ブランド名「{brand}」は絶対にタイトルに入れない（別欄で設定済み）
- 中点「・」は絶対に使わない（文字数の無駄）
- 「新作」「日本未入荷」など変動的なキーワード禁止
- 「関税なし」「関税免除」は絶対禁止
- 韓国語（ハングル文字: 가-힣）は絶対に使用禁止。1文字でも含めてはいけない
- ブランド名が韓国語の場合は英語またはカタカナに変換して使う

[フッキングのコツ]
- 購買意欲を刺激する言葉を入れる（例: 大人気、完売間近、話題、トレンド、SNSで話題）
- 季節感・シーン提案（例: 夏コーデ、通勤スタイル、デイリー使い）
- 素材・機能性（例: 軽量、ストレッチ、速乾、UVカット）"""
        else:
            prompt = f"""BUYMAの「色サイズ補足」欄に入る商品説明文を2000文字（半角基準）以内で作成。必ず全て日本語のみで作成。韓国語（ハングル文字）は絶対に使用禁止。ブランド名も韓国語ではなく英語またはカタカナで表記。説明文のみ出力（前置き不要）。
...
- カラー: {colors or '不明'}
- サイズ: {sizes or '不明'}
{f'- ユーザーからの追加指示: {keywords}（このキーワードを説明文に自然に含めること）' if keywords else ''}

[ルール]
- 韓国の現地バイヤーが直接買い付けた100%正規品を強調
- 商品の特徴・素材・着用感を記載
{f'- ユーザーからの追加指示「{keywords}」に従うこと' if keywords else ''}- 韓国人気のアピール
                ...
                try:
                    ...
                    try:
                        ...
                        if ai_cfg.get("provider", "none") != "none":
                            ...
                            title_prompt = f"""BUYMAの出品タイトルを半角56文字以内で作成。必ず全て日本語のみで作成。韓国語（ハングル文字）は絶対に使用禁止。ブランド名も韓国語ではなく英語またはカタカナで表記。タイトルのみ出力（説明不要）。
...
[タイトル作成ルール]
- タイトルにブランド名「{brand}」は入れない（別途ブランド欄があるため文字数を他のキーワードに使う）
- 商品の魅力が伝わるキーワードを最大限に入れる
- 注目キーワード活用: 【SALE】【即発】【韓国人気】【size:S】等
- 商品の特徴が伝わる魅力的な表現

[絶対禁止事項]
- ブランド名「{brand}」は絶対にタイトルに入れない（別欄で設定済み）
- 中点「・」は絶対に使わない（文字数の無駄）
- 「新作」「日本未入荷」など変動的キーワード禁止
- 「関税なし」「関税免除」は絶対禁止
- 韓国語（ハングル文字: 가-힣）は絶対に使用禁止。1文字でも含めてはいけない
- ブランド名が韓国語の場合は英語またはカタカナに変換して使う

[フッキングのコツ]
- 購買意欲を刺激する言葉（例: 大人気、完売間近、話題、トレンド、SNSで話題）
- 季節感・シーン提案（例: 夏コーデ、通勤スタイル、デイリー使い）
- 素材・機能性（例: 軽量、ストレッチ、速乾、UVカット）"""
                            title_ja = _try_ai(title_prompt)

                            if title_ja:
                                desc_prompt = f"""BUYMAの「色サイズ補足」欄に入る商品説明文を2000文字(半角)以内で作成。必ず全て日本語のみで作成。韓国語（ハングル文字）は絶対に使用禁止。ブランド名も韓国語ではなく英語またはカタカナで表記。
説明文のみ出力してください（前置き不要）。

【重要ルール】
- ブランド名は必ず「{brand}」を使用
- 韓国の現地バイヤーが直接買い付けた100%正規品であることを強調
- 商品の特徴・素材・着用感を記載
- 韓国人気・日本未入荷のアピール
- サイズ感・モニター環境による色味の注意事項
