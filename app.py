...
[絶対禁止事項]
- ブランド名は日本語・英語・韓国語いずれの表記でも絶対に入れない（「{brand}」「カルバンクライン」「Calvin Klein」「캘빈클라인」等すべて禁止。別欄で設定済み）
- 中点「・」は絶対に使わない（文字数の無駄）
...
[絶対禁止事項]
- ブランド名は日本語・英語・韓国語いずれの表記でも絶対に入れない（別欄で設定済みのため不要）
- 韓国語（ハングル文字）は絶対に使用禁止
- 中点「・」は使わない"""
...
【重要ルール】
- ブランド名は日本語・英語・韓国語いずれの表記でも絶対に入れない（別欄で設定済み）
...
【収集元の商品情報】
- 収集元タイトル: {src_title}
- ブランド: {brand}（※参考情報のみ。説明文にブランド名を入れないこと）
- 商品名: {title_ja}
- カラー: {color_str or '不明'}
- サイズ: {size_str or '不明'}"""
